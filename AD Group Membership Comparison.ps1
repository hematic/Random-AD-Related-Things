...
<#
.SYNOPSIS
    Compares the groups of the specified Active Directory users.
.DESCRIPTION
    Compare-ADGroups retrieves a list of all the Active Directory groups 
    that the specified users are a member of. It determines what groups are 
    common between the users based on membership of 50% or more of the 
    specified users. It then compares the specified users group membership 
    to the list of common groups and returns a list of users whose group
    membership differentiates from that list. A minus (-) in the status 
    column means the user is not a member of a common group and a plus (+) 
    means the user is a member of an additional group.
.PARAMETER UserName
...
.EXAMPLE
     Compare-ADGroups -UserName 'marshph', 'pandewa', 'HorowSt'
.EXAMPLE
     'jleverling', 'lcallahan', 'mpeacock' | Compare-ADGroups -IncludeEqual
.EXAMPLE
...
.NOTES
    ...
    Website: http://justanotherpsblog.com
#>
